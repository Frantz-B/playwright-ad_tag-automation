...
const { kargoLink, lighthouseAdLink, lighthouseDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Lighthouse Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(lighthouseDemoAdLink);
    await expect(page).toHaveURL(lighthouseDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    await expect(newPage).toHaveURL(lighthouseAdLink, {timeout: 10000}); // URL contains
  });
